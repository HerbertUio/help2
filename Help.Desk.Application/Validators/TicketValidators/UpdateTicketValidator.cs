...
public class UpdateTicketValidator : AbstractValidator<UpdateTicketDto>
{
    public UpdateTicketValidator()
    {
        ...
        // Validación: no puede estar asignado tanto a un agente como a un grupo
        RuleFor(x => x)
            .Must(x => !(x.AssignedAgentId.HasValue && x.AssignedGroupId.HasValue))
            .WithMessage("Un ticket no puede estar asignado tanto a un agente como a un grupo al mismo tiempo.")
            .WithName("AssignmentConflict");
    }
}